/**
 * UI — 概念目录(**已封存,当前没有任何地方挂载它**)
 *
 * ⚠️ 这不是死代码,是**停放**的代码。
 * Jesse 要求把首页清空好放新东西,但八条链本身没有下线 ——
 * `#/limits`、`#/log-integral` 这些路由照常能开,只是首页不再列出来。
 * 所以这个文件原样保留:hero、四条学习轨道、八张卡片、缩略图映射。
 * 想让目录回来,只要在 `Home.tsx` 里 `import { ConceptGrid }` 再渲染一次。
 *
 * 因为没人 import,Rollup 会把它整个 tree-shake 掉,
 * 连带那九张 `new URL(...)` 的图片也不会进产物 —— 停放不占体积。
 *
 * `registry.test.ts` 仍然盯着下面的 THUMBNAILS:新增一条链却忘了配图,
 * 在目录回归的**那一刻**就会炸(W7 就是这么把首页整个搞白的)。
 * 与其等到那时候再想起来,不如现在就让测试红。
 */
...
export const THUMBNAILS: Readonly<Record<string, { src: string; alt: string }>> = {
  'log-integral': {
...
/** 封存的目录本体。恢复方式:在 Home 里渲染 `<ConceptGrid />`。 */
export function ConceptGrid({ concepts = CONCEPTS }: { concepts?: readonly ConceptMeta[] }) {
  return (
    <>
      <HeroSlot />
